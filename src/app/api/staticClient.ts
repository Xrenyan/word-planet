...
export function createStaticWordPlanetApi(options: StaticClientOptions = {}): WordPlanetApi {
  ...
  async function requestJson(path: string, signal?: AbortSignal) {
    const response = await fetcher(`${baseUrl}${path}`, { headers: { accept: 'application/json' }, signal })
  ...
  function getBook(bookId: string, signal?: AbortSignal) {
    ...
    const request = requestJson(`data/books/${encodeURIComponent(bookId)}.json`, signal)
  ...
  return {
    getBooks(signal) {
      catalogRequest ??= requestJson('data/catalog.json', signal)
    ...
    async getReview(profileId, signal) {
      const [catalog, stats] = await Promise.all([
        ...
        Promise.resolve(progress.reviewStats(profileId)),
      ])
      ...
      return {
        ...
        items: stats.flatMap((stat) => {
          ...
          return word ? [{ word, misses: stat.misses, correct: stat.correct, weakness: stat.weakness, lastAttemptAt: stat.lastAttemptAt }] : []
        }),
      }
    },
