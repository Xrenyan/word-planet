...
type BubbleMatchGameProps = GameCompletionCallbacks & {
...
export function BubbleMatchGame({ words, rounds = 3, seed = 20260818, ...props }: BubbleMatchGameProps) {
...
function BubbleMatchSession({ gameRounds, onReturnToLearning, onBackToHub, onAttempt, onComplete, onReplay, onNextChallenge }: BubbleMatchSessionProps) {
  ...
  // Commit event state synchronously so batched clicks cannot overwrite an earlier miss.
  const sessionRef = useRef({ roundIndex: 0, state, completed: [] as readonly GameRoundState[], finished: false })
  ...
  useEffect(() => {
    ...
    onComplete({ completedRounds: result.rounds, totalRounds: gameRounds.length, firstTryCorrect: result.correctRounds })
  }, [finished, completedRounds, gameRounds.length, onComplete])

  function answer(wordId: string) {
    const session = sessionRef.current
    if (session.finished || session.roundIndex !== roundIndex || session.state.status === 'complete') return
    const next = recordGameAnswer(session.state, wordId)
    session.state = next
    setState(next)
    if (next.status === 'complete' && roundIndex + 1 >= gameRounds.length) {
      session.completed = Object.freeze([...session.completed, next])
      session.finished = true
      setCompletedRounds(session.completed)
      setFinished(true)
    }
  ...
  function nextRound() {
    const session = sessionRef.current
    if (session.finished || session.roundIndex !== roundIndex || session.state.status !== 'complete') return
    session.completed = Object.freeze([...session.completed, session.state])
    setCompletedRounds(session.completed)
    const nextIndex = roundIndex + 1
    session.roundIndex = nextIndex
    session.state = createGameRoundState(gameRounds[nextIndex])
  ...
  if (finished) {
    ...
    return (
      <section className="bubble-game bubble-game--complete" aria-labelledby="bubble-game-title">
        ...
        <p className="bubble-game__result">完成 {result.rounds} / {gameRounds.length} 轮</p>
        <p>首次答对 {result.correctRounds} / {result.rounds} 轮</p>
        <p>泡泡都找对啦！再选个游戏，或回去学单词吧。</p>
        {onReplay && <button className="game-back" type="button" onClick={onReplay}>再玩一次</button>}
        {onNextChallenge && <button className="bubble-game__return" type="button" onClick={onNextChallenge}>下一关 <ArrowRight aria-hidden="true" weight="bold" /></button>}
