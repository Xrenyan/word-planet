...
test('loads the complete static curriculum and opens a real word', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '三年级上册' })).toBeVisible()
  ...
  const learnedWord = await page.locator('.verified-practice__word-card strong').textContent()
  await page.getByRole('button', { name: '返回词表' }).click()
  await expect(page.locator('.curriculum-book-view__focus h3')).toHaveText(learnedWord!)
...
test('keeps key tools usable at a phone viewport', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '换设备继续学' })).not.toBeVisible()
  ...
  await page.getByText('家长工具', { exact: true }).click()
  await expect(page.getByRole('heading', { name: '换设备继续学' })).toBeVisible()
